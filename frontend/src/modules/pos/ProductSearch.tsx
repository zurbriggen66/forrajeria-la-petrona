    ...
    <div className="relative">
      <div className="relative">
        ...
        <input
          ...
          placeholder="Escaneá o buscá por nombre, código o PLU…"
        ...
        <div className="absolute z-20 mt-1 w-full overflow-hidden rounded-xl border border-border bg-surface shadow-xl">
          {resultados.map((p) => (
            <button
              key={p.id}
              onClick={() => agregar(p)}
              className="flex w-full items-center justify-between gap-3 border-b border-border px-4 py-2.5 text-left last:border-0 hover:bg-surface-2"
            >
              <div>
